...
export class ConversationRepository extends Repository<Conversation> {
  async getConversation(
    ...
    if (receiverId !== null) {
      query
        .andWhere('(conversation.senderId=:senderId and conversation.receiverId=:receiverId)', {
          senderId,
          receiverId,
        })
        .orWhere('(conversation.senderId=:receiverId and conversation.receiverId=:senderId)', {
          receiverId,
          senderId,
        });
    } else {
      query.andWhere('conversation.senderId=:senderId or conversation.receiverId=:senderId', {
        senderId,
      });
    }

    if (filter.after) {
      query.andWhere('conversation.createdAt > :after', { after: filter.after });
    }

    if (filter.before) {
      query.andWhere('conversation.createdAt < :before', { before: filter.before });
    }
    if (filter.page) {
      if (filter.limit) {
    ...
    return query.orderBy('conversation.createdAt', 'ASC').getMany();
  }
